...
def main():
    ...
    class_list = config['CLASSES'].split(',')

    #Iterate files and generate feature vectors.
    file_sentence_dict = create_annotated_sentence_structures(config['ANNOTATION_FILE_PATH'], config['RAW_FILE_PATH'])
    add_modified_sentence_array(file_sentence_dict)
    tx, ty, ts, tm = generate_embeddings(file_sentence_dict, config, feature_map)
    train_batch_container = BatchContainer(tx, ty, ts, tm)

    #Train the network.
    train_network(train_batch_container, file_sentence_dict, int(config['EMBEDDING_SIZE'])+len(feature_map), class_list, config)

#Author: Jeffrey Smith
def train_network(train_batch_container, file_sentence_dict, num_features, class_list, config, supplemental_batch=None):
    """
    ...
    :param num_features: Number of features to include as input into the network.
    :param class_list: A list of the classes to be classified.
    ...
    for k in range(0, buckets):
        trainer = agent.Agent(num_features, 4, int(config['MAX_SENTENCE_LENGTH']))
...
#Author: Jeffrey Smith
def generate_embeddings(file_sentence_dict, config, feature_map):
    """
    ...
    #Launch perl pipe
    args = ['perl', './w2v.pl', config['EMBEDDING_FILE']]
    ...
    try:
        ...
        for k_ in k:
            ...
            for x in file_sentence_dict[k_]:
                f.write("START\n")
                t_array = np.zeros((int(config['MAX_SENTENCE_LENGTH']), int(config['EMBEDDING_SIZE']) + len(feature_map)), dtype=np.float32)
                c_array = np.zeros((int(config['MAX_SENTENCE_LENGTH'])), dtype=np.float32)
                ...
                for z in range(0, len(x.modified_sentence_array)):
                    ...
                    while not p.poll():
                        ...
                        if "UNDEF" in t:
                            ...
                            t_split = t.split()
                            t_array[z][0:int(config['EMBEDDING_SIZE'])] = t_split[1:int(config['EMBEDDING_SIZE'])+1]
                            c_array[z] = class_
                    ...
                    if x.modified_sentence_array[z][0] == ":":
                        t_array[z][int(config['EMBEDDING_SIZE']) + feature_map["PUNC_OTHER"]] = 1.0
                    elif x.modified_sentence_array[z][0] == ";":
                        t_array[z][int(config['EMBEDDING_SIZE']) + feature_map["PUNC_OTHER"]] = 1.0
                    elif x.modified_sentence_array[z][0] == ",":
                        t_array[z][int(config['EMBEDDING_SIZE']) + feature_map["PUNC_COMMA"]] = 1.0
                    elif x.modified_sentence_array[z][0] == ".":
                        t_array[z][int(config['EMBEDDING_SIZE']) + feature_map["PUNC_PERIOD"]] = 1.0
                        ...
                        t_array[z][int(config['EMBEDDING_SIZE']) + feature_map["IS_NUM"]] = 1.0
                    elif x.modified_sentence_array[z][0] == "date":
                        t_array[z][int(config['EMBEDDING_SIZE']) + feature_map["IS_DATE"]] = 1.0
